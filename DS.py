...
class DraggableItem(QFrame):
    ...
    def toggle_favorite(self):
        ...
        # If we unfavorite something that was already hidden from main, it should probably show up again or be deleted?
        # Logic: Unfavoriting restores it to main list (if it was hidden) so it doesn't vanish entirely
    ...
    def request_removal(self):
        # Logic: 
        # If in 'All Items' tab and is favorite: Hide from main, keep in favorites.
        # If in 'Favorites' tab: Unfavorite (and delete if not in main).
        if self.parent_shelf:
    ...
    def handle_copy(self):
        ...
        if self.data_type == 'file':
            # Copy file object so it can be pasted in Explorer
            url = QUrl.fromLocalFile(self.content)
            mime_data.setUrls([url])
            mime_data.setText(self.content) # Fallback text
        else:
...
class SettingsDialog(QDialog):
    def __init__(self, current_hotkey, parent=None):
        ...
        layout.addWidget(QLabel("Global Hotkey to Toggle Shelf:"))
        
        # Use recording widget instead of plain line edit
        self.hotkey_input = HotkeyRecorder(current_hotkey)
        layout.addWidget(self.hotkey_input)
        
        layout.addWidget(QLabel("(Click box and press keys)"))
...
class DropShelfWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.btn_close = create_header_btn("✕", "Exit App", self.close_app)
        ...
        for i in range(self.scroll_layout.count()):
            ...
            if isinstance(widget, DraggableItem):
                ...
                if self.current_tab == "fav":
                    ...
                    # All Items tab
                    # Show if it's NOT hidden from main
    ...
    def handle_item_deletion_request(self, item):
        # 1. If we are in Favorites tab: User explicitly wants to remove this favorite.
        # Action: Remove favorite status. If also hidden from main, delete permanently.
        if self.current_tab == "fav":
            item.is_favorite = False
            item.update_style()
            item.update_star_style()
            
            # If it's not visible in main either, delete it fully
            if item.hidden_from_main:
                self.remove_item(item)
            else:
                # Just refresh so it disappears from favorites list
                self.refresh_visibility()
                self.save_favorites()
        
        # 2. If we are in All Items tab:
        else:
            if item.is_favorite:
                # User wants to clear it from "Clipboard" view, but keep it in Favorites.
                item.hidden_from_main = True
                self.refresh_visibility()
                self.save_favorites() # Save state (hidden flag is persisted)
            else:
                # Not a favorite, just delete it
                self.remove_item(item)
    ...
    def open_settings(self):
        dlg = SettingsDialog(self.hotkey, self)
        if dlg.exec():
    ...
    def clear_shelf(self):
        # Logic: If item is favorite, set hidden_from_main = True
        # If item is NOT favorite, remove permanently
        to_remove = []
    ...
    def dragEnterEvent(self, event):
        # Accept internal reordering OR external files/text
    ...
    def dropEvent(self, event):
        ...
        # 1. Handle Internal Reorder
        if mime.hasFormat("application/x-dropshelf-item"):
            try:
                # We passed the memory ID of the object
                ...
                if source_widget:
                    ...
                    # Map position to scroll content coordinates
                    ...
                    if insert_index == -1:
                        insert_index = self.scroll_layout.count() - 1 # End
    ...
    def add_item(self, dtype, content, is_favorite=False, hidden_from_main=False):
        # Auto-switch to All tab if adding new content so user sees it
        ...
        # If exists, remove it first (so we can add it to the top)
        if existing_widget:
            # Preserve state if exists
            if not is_favorite and existing_widget.is_favorite:
                is_favorite = True
            
            # If we re-add something that was hidden from main, unhide it!
        ...
        self.scroll_layout.insertWidget(0, item) # Insert at top
        
        self.refresh_visibility()
        
        # Only save if we changed structure from user action (not loading)
        ...
        try:
            with open(FAVORITES_FILE, 'r') as f:
                ...
                for fav in reversed(favorites):
                    # Handle legacy format where hidden_from_main didn't exist
